...
export default function Men() {
  ...
  const sorted = [...MEN].sort((a,b)=>sort==="price_asc"?a.price-b.price:sort==="price_desc"?b.price-a.price:sort==="rating"?b.rating-a.rating:0);

  const handleAdd = (e,p)=>{ e.stopPropagation(); addToCart(p,p.sizes?.[2]||null,null); setAdded(v=>({...v,[p.id]:true})); setTimeout(()=>setAdded(v=>({...v,[p.id]:false})),1600); };
  ...
  return (
    <div className="min-h-screen bg-[#080808] pt-20">
      ...
      <div className="relative h-[48vh] overflow-hidden">
        <img src="https://images.unsplash.com/photo-1617127365659-c47fa864d8bc?w=1600&q=80" alt="Men" className="w-full h-full object-cover object-top" style={{filter:"brightness(0.3) saturate(0.6)"}}/>
        <div className="absolute inset-0 bg-gradient-to-t from-[#080808] via-transparent to-transparent"/>
        <div className="absolute inset-0 flex flex-col items-center justify-center text-center gap-3">
          <p className="text-amber-400 text-[11px] tracking-[0.3em] uppercase font-bold">Collection</p>
          <h1 className="text-5xl md:text-6xl font-black text-white font-serif">Men's Edit</h1>
          <p className="text-white/35 text-base max-w-md">Refined essentials for the modern man. Built to last.</p>
        </div>
      </div>
          ...
          {sorted.map(p=>(
            <div key={p.id} onClick={()=>navigate(`/product/${p.id}`)} className="group bg-[#111] rounded-2xl overflow-hidden border border-white/[0.06] hover:border-amber-400/20 cursor-pointer transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_30px_70px_rgba(0,0,0,0.7)]">
              <div className="relative overflow-hidden aspect-[4/5]">
                ...
                <span className="absolute top-3 left-3 bg-sky-400 text-black text-[9px] font-black px-2 py-0.5 rounded-full">{p.badge}</span>
                <span className="absolute top-3 right-10 bg-black/65 text-white text-[9px] font-black px-2 py-0.5 rounded-full">−{pctOff(p.original,p.price)}%</span>
